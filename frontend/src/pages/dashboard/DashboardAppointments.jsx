...
const STATUS_MAP = {
  ...
  pending: { label: 'Pendiente', icon: AlertCircle, color: 'text-amber-500 bg-amber-500/10 border-amber-500/20' },
  completed: { label: 'Completada', icon: CheckCircle2, color: 'text-blue-500 bg-blue-500/10 border-blue-500/20' },
  cancelled: { label: 'Cancelada', icon: XCircle, color: 'text-red-500 bg-red-500/10 border-red-500/20' },
};
...
export default function DashboardAppointments() {
  ...
  const enriched = allAppointments.map(a => ({
    ...a,
    ...
    <div className="space-y-6">
        ...
        <div className={`transition-all duration-700 delay-200 ${visible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-6'}`}>
          ...
          <div className="space-y-3">
            {upcoming.map((a, i) => {
              ...
              const Icon = st.icon;
              return (
                <div key={a.id} className="card-elevated rounded-2xl p-5 transition-all duration-500 hover:shadow-lg" style={{ transitionDelay: `${250 + i * 80}ms` }}>
                  <div className="flex items-start gap-4">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-xs text-cyan-500 font-medium mb-2">{a.specialty}</p>
                      <div className="flex flex-wrap items-center gap-x-4 gap-y-1 text-xs text-muted">
                        ...
                        {a.doctor?.location && <span className="flex items-center gap-1"><MapPin size={12} />{a.doctor.location}</span>}
                      </div>
                    </div>
                  </div>
                </div>
        ...
        <div className={`transition-all duration-700 delay-300 ${visible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-6'}`}>
          ...
          <div className="space-y-3">
            {past.map((a, i) => {
              ...
              const Icon = st.icon;
              return (
                <div key={a.id} className="card-elevated rounded-2xl p-5 opacity-70 transition-all duration-500 hover:opacity-100" style={{ transitionDelay: `${350 + i * 60}ms` }}>
      ...
      {filtered.length === 0 && (
        <div className="text-center py-16">
          <Calendar size={36} className="text-muted mx-auto mb-4" />
          ...
          <Link to="/appointments/new" className="btn-primary px-5 py-2.5 rounded-xl text-sm font-semibold inline-flex items-center gap-2">
