...
import {
  Stethoscope, Star, MapPin, Search, User,
  Clock, ChevronRight
} from 'lucide-react';
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
        {filtered.map((doc, i) => (
          <div
            key={doc.id}
            className={`card-elevated rounded-2xl p-5 transition-all duration-500 hover:shadow-lg group ${visible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-6'}`}
          ...
          >
            <div className="flex items-start gap-4 mb-4">
              ...
              <div className="min-w-0 flex-1">
                <h3 className="font-bold text-heading text-sm truncate group-hover:text-blue-500 transition-colors">{doc.name}</h3>
                <p className="text-xs text-cyan-500 font-medium">{doc.specialty}</p>
            ...
            <div className="flex items-center justify-between pt-3 border-t border-themed">
              <div>
                <p className="text-emerald-500 font-bold text-sm">{doc.price}</p>
                <p className="text-[10px] text-muted flex items-center gap-1"><Clock size={10} />{doc.nextAvailable}</p>
              </div>
              <Link
      ...
      {filtered.length === 0 && (
        <div className="text-center py-16">
          <Stethoscope size={36} className="text-muted mx-auto mb-4" />
